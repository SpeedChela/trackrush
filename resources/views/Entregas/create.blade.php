            ...
            <form action="{{ route('entregas.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('estado_actual') is-invalid @enderror" 
                        ...
                        <option value="Pendiente" {{ old('estado_actual') == 'Pendiente' ? 'selected' : '' }}>Pendiente</option>
                        <option value="En Proceso" {{ old('estado_actual') == 'En Proceso' ? 'selected' : '' }}>En Proceso</option>
                        <option value="Completada" {{ old('estado_actual') == 'Completada' ? 'selected' : '' }}>Completada</option>
                        <option value="Cancelada" {{ old('estado_actual') == 'Cancelada' ? 'selected' : '' }}>Cancelada</option>
                    </select>
                    @error('estado_actual')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
